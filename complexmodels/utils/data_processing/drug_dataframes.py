...
def get_drug_fgpt_df(data_root: str,
    ...
    # Otherwise load from raw files, process it and save ######################
        ...
        print(os.path.join(data_root, RAW_FOLDER, PFP_FILENAME))
...
def get_drug_prop_df(data_root: str):
    ...
    # Otherwise load from raw files, process it and save ######################
        ...
        # Note that drug target file is not downloadable
        raw_file_path = os.path.join(data_root, RAW_FOLDER, DRUG_PROP_FILENAME)
        if not os.path.exists(raw_file_path):
            raise FileNotFoundError(
                'Drug target file %s does not exist. '
                'Please download from github repo directly.'
                % DRUG_PROP_FILENAME)
